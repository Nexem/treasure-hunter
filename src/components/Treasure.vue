  ...
  methods: {
    ...
    moveAdventurer() {
      ...
      for (var adventurer = 0; adventurer < nbAdventurer; adventurer++) {
        ...
        this.indexOrientation =
          this.listOrientation[caracteristics.orientation];
        ...
        for (var action = 0; action < nbactions; action++) {
          ...
          caracteristics.orientation =
            this.listOrientation[this.indexOrientation];
    ...
    checkMove(x, y, orientation) {
      switch (orientation) {
        ...
        case "E":
          y += 1;
          if (y <= this.map[x].length && this.map[x][y] !== "M") {
            console.log("ok", this.map[x][y]);
            this.positionY += 1;
            if (this.map[x][y].entity && this.map[x][y].entity === "T") {
              this.map[x][y].value -= 1;
              this.map[x][y - 1].tresor += 1;
              if (this.map[x][y].value === 0) {
                this.map[x][y] === ".";
              }
            }
